...
uint16_t get_tap_flow(uint16_t keycode, keyrecord_t* record, uint16_t prev_keycode) {
    if (HT_THUMBS_IF(keycode) || HT_THUMBS_IF(prev_keycode)) { return 0; } else { return g_tap_flow_term; };
}
